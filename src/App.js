...
class App extends React.Component {
  ...
  createUser = async () => {
    ...
    const { id,name,email,company_name} = this.state;
        try {
            const { data: user} = await axios.post('https://jsonplaceholder.typicode.com/users', { id,name,email,company_name })
            const users = [...this.state.users]
            users.push(user)
            this.setState({ users, id: "", name: "", email: "",company_name })
    ...
    return (
      <>
        <Box component="form" onSubmit={this.handleSubmit}
          ...
          autoComplete="off">
          <h3> Create User:</h3>
          <TextField id="outlined-password-input" label="Id" type='text' name="id" value={this.state.id} onChange={this.handleChange} /><br />
